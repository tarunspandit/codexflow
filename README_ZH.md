...
codexflow 会从本机 Codex metadata 自动发现所有项目，启动 broker 和 Cloudflare tunnel，安装并打开原生 macOS 或 Windows 应用，同时复制 ChatGPT Server URL。先到 `Settings -> Security and login` 打开 Developer mode，再到 `Settings -> Plugins` 创建连接，粘贴这个 URL，并选择 `Authentication: No Authentication / None`。
...
在 macOS 14 或更新版本，第一次运行 `codexflow` 会把随包发布的原生应用安装到 `~/Applications`。在 Windows 10/11，它会把匹配架构的已校验、自包含应用安装到 `%LOCALAPPDATA%\CodexFlow`。两者都会自动打开。之后即使 broker 尚未运行，也可以在 CodexFlow 终端按 `o`，或从另一个终端运行：
...
当项目、Git 和终端工具不足时，Web chat 可以通过 `computer_use` 请求一个明确的原生 macOS 或 Windows 应用。请在原生 **Computer** 页面允许一次或持久授权，并在本机确认每个 press、文本或按键操作。授权绑定目标应用身份；macOS 使用 Accessibility/Screen Recording 与代码签名，Windows 使用 UI Automation、仅目标窗口截图与发布者或二进制身份。Terminal、ChatGPT/CodexFlow、系统设置、浏览器应用、安全输入框与疑似 secret 文本都会被拒绝。

网站操作使用独立的 `browser_use` 边界。聊天先请求一个准确的 HTTP(S) origin；原生 **Browser** 页面可以拒绝、为该 route 允许十分钟，或持久允许直到撤销。页面在 macOS 的临时 WebKit tab 或 Windows 的临时 WebView2 tab 中打开，不使用你的个人浏览器 profile。聊天只能获得最新截图与受限的语义 DOM 目标；click、文本和敏感按键都绑定 route、tab、snapshot、element、operation 与 value，并需要本机确认。跨 origin 跳转、popup、下载、浏览器权限、登录/账户安全/支付页面、密码值、URL 凭据与疑似 secret 输入都会 fail closed。
