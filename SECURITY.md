...
- bounded subagent names, roles, states, details, and results explicitly reported through `agent_progress` after ChatGPT Work creates the child
...
- optional native macOS or Windows Computer Use for one explicitly approved, identity-bound app, with fresh app-only window captures and bounded Accessibility/UI Automation metadata
- optional native browser use for one explicitly approved HTTP(S) origin, with an ephemeral WebKit or WebView2 profile, fresh screenshots, bounded semantic DOM metadata, exact-element user comments, and query-free console/resource/source diagnostics private to the owning route
...
| Subagent coordination leaks a usable child route or allows cross-child mutation | `agent_progress` returns child route credentials only to the authorized MCP caller; native/API telemetry omits them. A child route is mapped to one parent and one agent, may update only that record, and is revoked from the persistent route store when the parent clears coordination. Parent/agent records are bounded, secret-checked, and owner-only. CodexFlow allocates routes but never starts or impersonates a model agent. |
...
| A chat silently captures or controls an arbitrary app | Computer Use requires OS-native access (Screen Recording/Accessibility on macOS or UI Automation on Windows), then a second CodexFlow-native app grant. Windows captures only the approved window via `PrintWindow`, not arbitrary screen coordinates. Allow-once grants are route-private and expire; persistent grants are stored owner-only and can be revoked. |
| A different binary reuses an approved app identifier | Every app listing, snapshot, and action validates target identity. macOS binds approval to signing identifier, team, and code-directory hash; Windows binds to publisher certificate plus canonical executable path, or the complete binary hash when unsigned. An identity change fails closed and requires fresh approval. |
...
| Browser state leaks personal sessions or credentials | Every tab uses a non-persistent WebKit or WebView2 data store rather than Safari/Edge/Chrome state. URL credentials, secret-looking query values, password contents/input, authentication hosts, and account-security, billing, checkout, and payment paths fail closed. |
