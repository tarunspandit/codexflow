...
| Browser developer diagnostics | Partial | Semantic DOM inspection, screenshots, visible rendering, exact-element comments, and bounded action diagnostics are available. A general Chrome DevTools Protocol console/network/source inspector is not implemented because the native browser is WebKit and intentionally exposes a narrower approval surface. |
...
2. Full developer diagnostics comparable to Codex desktop’s developer mode, if that broad inspection surface can be added without weakening the explicit origin/action consent model.
3. A live authenticated ChatGPT Work acceptance run proving that inherited CodexFlow tools reach two simultaneously spawned children, each child retains its allocated route/worktree, the parent can steer and consolidate, and native Active/Done state follows the hosted thread lifecycle.
