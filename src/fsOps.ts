...
import { CodexFlowError, displayPath, normalizeRelPath, PathGuard } from "./guard.js";
...
export function textScanByteLimit(config: CodexFlowConfig): number {
...
export async function repoTree(config: CodexFlowConfig, guard: PathGuard, workspace: Workspace, options: TreeOptions): Promise<TreeResult> {
  const target = guard.resolve(workspace, options.path ?? ".");
  const stat = await fsp.stat(target.absPath);
  if (!stat.isDirectory()) {
    throw new CodexFlowError(`Not a directory: ${target.relPath}`);
  }
...
export async function readTextFile(
  ...
  if (endLine < startLine) {
    throw new CodexFlowError(`end_line (${endLine}) must be >= start_line (${startLine}).`);
  }
  ...
  if (hasRange && Buffer.byteLength(numbered, "utf8") > maxBytes) {
    throw new CodexFlowError(`Selected line range is too large. Limit: ${maxBytes} bytes.`);
  }
...
export async function writeTextFile(
  ...
  if (contentBytes > config.maxWriteBytes) {
    throw new CodexFlowError(`Write content is too large (${contentBytes} bytes). Limit: ${config.maxWriteBytes} bytes.`);
  }
  if (hasSecretValue(content)) {
    throw new CodexFlowError("Secret-looking content is blocked from write. Use placeholders such as [REDACTED_SECRET] in handoff files.");
  }
  ...
  try {
    ...
    if (error instanceof CodexFlowError && error.message.startsWith("Not a file")) throw error;
    if (fs.existsSync(resolved.absPath)) throw error;
  }

  if (existed && options.overwrite === false) {
    throw new CodexFlowError(`File already exists and overwrite=false: ${resolved.relPath}`);
  }
...
export async function editTextFile(
  ...
  if (!oldText) throw new CodexFlowError("old_text must not be empty.");
  const resolved = guard.resolve(workspace, filePath, { forWrite: true });
  await guard.assertTextFile(resolved.absPath, Math.max(config.maxWriteBytes, config.maxReadBytes));
  const before = await fsp.readFile(resolved.absPath, "utf8");
  const occurrences = before.split(oldText).length - 1;
  if (occurrences === 0) {
    throw new CodexFlowError(`old_text was not found in ${resolved.relPath}. Read the file and retry with an exact snippet.`);
  }
  ...
  if (options.replaceAll) {
    ...
    if (occurrences !== 1) {
      throw new CodexFlowError(`old_text matched ${occurrences} times. Provide a more specific old_text or set replace_all=true.`);
    }
  ...
  if (typeof options.expectedReplacements === "number" && replacements !== options.expectedReplacements) {
    throw new CodexFlowError(`Expected ${options.expectedReplacements} replacements but would perform ${replacements}.`);
  }

  const afterBytes = Buffer.byteLength(after, "utf8");
  if (afterBytes > config.maxWriteBytes) {
    throw new CodexFlowError(`Edited file would be too large (${afterBytes} bytes). Limit: ${config.maxWriteBytes} bytes.`);
  }
  if (hasSecretValue(after)) {
    throw new CodexFlowError("Secret-looking content is blocked from edit. Use placeholders such as [REDACTED_SECRET] in handoff files.");
  }
...
export async function ensureAiBridge(config: CodexFlowConfig, guard: PathGuard, workspace: Workspace): Promise<string[]> {
