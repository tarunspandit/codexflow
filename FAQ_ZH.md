...
## CodexFlow 会使用与 Codex 相同的本地环境吗？

会。CodexFlow 直接读取 version-1 `.codex/environments/*.toml`，包括平台专用的 setup/cleanup 脚本和命名 actions。受管 worktree 可以自动运行选定环境的 setup，并复制 `.worktreeinclude` 匹配的 gitignored 文件。两者共享配置格式，但 CodexFlow 通过自己的受保护本地 broker 执行，绝不会启动 Codex CLI。

## ChatGPT Scheduled 能定时处理本地项目吗？

可以，通过 ChatGPT Scheduled 与 CodexFlow plugin。让 ChatGPT 准备并安排任务；`prepare_scheduled_task` 会生成稳定提示，每次运行重新获得项目 route，并可使用新的受管 worktree。电脑需要保持唤醒，broker 必须运行，plugin URL 必须稳定。模型运行与 schedule 属于 ChatGPT；CodexFlow 不会运行隐藏模型或本地 cron daemon。
