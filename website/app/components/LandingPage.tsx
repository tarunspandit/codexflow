...
const content = {
  en: {
    ...
    heroBody: "CodexFlow gives ChatGPT a serious local coding backend. Projects, managed worktrees, guarded Git workflows, persistent terminals, skills, and instructions become available through one deliberate connection.",
    copy: "Copy",
    copied: "Copied",
    ...
    companionBody: "CodexFlow opens as a native app on your Mac. Start and stop the broker, create isolated worktrees, organize routed chats, copy the connection, and edit next-launch policy without managing browser tabs.",
    companionLive: "Live on this computer",
    companionViews: ["Now", "Projects", "Worktrees", "Chats", "Connection", "Policy"],
    companionMetrics: [["Projects", "Discovered"], ["Worktrees", "Isolated"], ["Chat routes", "Independent"], ["Terminal", "Route-persistent"]],
  ...
  zh: {
    ...
    hero: ["一个命令。", "所有项目。", "任意对话。"],
    heroBody: "CodexFlow 为 ChatGPT 提供可靠的本地编码后端。项目、受管 worktrees、受保护 Git 流程、持久终端、skills 与指令，都通过一条清晰可控的连接进入对话。",
    copy: "复制",
    copied: "已复制",
    seePath: "查看完整流程",
    ...
    companionBody: "CodexFlow 会在 Mac 上打开原生应用。启动或停止 broker、创建隔离 worktrees、整理聊天路由、复制连接和编辑下次启动策略，都无需管理浏览器标签。",
    companionLive: "正在此电脑上运行",
    companionViews: ["当前", "项目", "Worktrees", "聊天", "连接", "策略"],
    companionMetrics: [["项目", "自动发现"], ["Worktrees", "彼此隔离"], ["聊天路由", "各自独立"], ["终端", "随路由持久"]],
    ...
    safetyLink: "阅读安全模型",
    guards: [["Token 保护的公网 URL", "未经身份验证的请求无法进入工具面。"], ["工作区 root 强制执行", "路径与 symlink 检查让所有操作始终在范围内。"], ["Secret 感知输出", "凭据和常见 secret 模式会被拦截或隐藏。"], ["受保护写入与终端", "安全默认值一直生效，直到你明确扩大权限。"], ["冲突感知 Worktrees", "交接前会验证两个 checkout，避免覆盖独立改动。"]],
    faqKicker: "直接回答",
    faqTitle: "连接之前。",
    ...
    <div className="site-page" lang={locale === "zh" ? "zh-CN" : "en"}>
      ...
      <main id="main">
        <section className="hero" id="top">
          ...
          <div className="hero-foot section-shell" aria-hidden="true"><span>CODEXFLOW / 0.31.0</span><span>LOCAL MACHINE → CHATGPT</span><span>SCROLL TO ENTER</span></div>
        </section>
