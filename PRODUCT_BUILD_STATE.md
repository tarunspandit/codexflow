...
Version 0.43.0 is in release verification. It adds route-scoped coordination for subagents that ChatGPT Work actually spawns: the parent allocates independent project routes, children update only their own bounded records, native Tasks mirrors Active/Done state, and child activity rolls into the parent without exposing usable route credentials. CodexFlow still does not spawn or proxy models; ChatGPT remains the conversation and model surface. Focused runtime/HTTP regressions and signed universal-app build, launch, WebKit, accessibility-tree, and visual checks pass. Full release, public deployment, installation, and live authenticated ChatGPT Work acceptance remain in progress.
...
| Computer Use boundary | Separate macOS helper plus broker-owned approvals | Native visual tasks need explicit local consent without allowing a model to bypass terminal, browser-host, or ChatGPT approval boundaries. | Grants bind to route and validated code-signing identity; snapshots expire; sensitive actions require exact local confirmation. |
...
- Root TypeScript build, complete smoke suite, stress suite, signed universal-app build/smoke, package dry-run, production audit, and bilingual website lint/build/render/audit pass for 0.42.0.
...
- The 0.42.0 public website describes live web-task progress, managed SSH worktrees, guarded handoff, per-hunk review, web-chat-visible line notes, permissioned Computer Use, and isolated native Browser control; English/Chinese output is verified on GitHub Pages and the Flow7-hosted production deployment.
...
- The existing public URLs remain `https://tarunspandit.github.io/codexflow/` and `https://codexflow.tarunspandit.chatgpt.site/`; both were verified serving the 0.42.0 Tasks release in English and Chinese.
...
- npm registry publication requires a refreshed maintainer login (`npm whoami` returned E401 during the 0.42.0 release on 2026-07-23). This does not block source, desktop, GitHub release, GitHub Pages, Sites, or permanent-tunnel operation.
...
0.43.0 implementation and focused verification are complete. Full smoke/stress/package/audit, public builds, GitHub and Sites deployment, installed-app refresh, and live tunnel checks remain before release handoff. npm publication and authenticated ChatGPT Work acceptance are external account checks. Literal cross-platform parity still requires a Windows-native management/approval client.
