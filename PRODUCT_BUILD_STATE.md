...
The 0.40.1 source, signed native app, GitHub release, GitHub Pages deployment, and Flow7-hosted site are live. Permissioned native Computer Use lets a routed chat request one signed macOS app, observe a fresh window snapshot, and perform locally confirmed element-targeted actions without arbitrary coordinate control. The native Changes workspace supports stable content-derived hunk actions and line-anchored review notes; approved OpenSSH projects retain route-private terminals, environments, skills, analysis, and guarded managed worktrees. Literal Codex desktop parity is not claimed; dedicated per-host browser control, richer native task progress/orchestration, and native clients beyond macOS remain tracked in `CODEX_DESKTOP_PARITY.md`. npm publication and live ChatGPT scheduled/two-chat acceptance remain account-authenticated checks.
...
| FUNC-003 | Native Now, Projects, Environments, Worktrees, Changes, Chats, Hosts, Computer, Connection, and Policy views render real broker/project data. | Required | Verified | Native views + authenticated broker APIs | Fixture and API regression coverage includes all destinations. |
...
| UI-001 | The native IA is Now / Projects / Environments / Worktrees / Changes / Chats / Hosts / Computer / Connection / Policy, composed from first principles for desktop. | Required | Verified | Native navigation shell | All destinations build and fixture/API states are covered by native smoke checks; the navigation remains scrollable at minimum height. |
...
| Native information architecture | Now / Projects / Environments / Worktrees / Changes / Chats / Hosts / Computer / Connection / Policy | Maps directly to “what is live, where, how projects initialize, which checkouts exist, what changed, which hosts and apps are trusted, who is connected, how to connect, what is allowed.” | No fake chat composer or redundant model experience. |
