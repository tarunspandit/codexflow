...
The released 0.42.0 build turns the native Chats surface into a real Tasks workspace. Every project-routed web chat can publish a bounded plan, current focus, blocker, review state, and completion snapshot through `task_progress`; the native app aggregates parallel work while ChatGPT remains the sole model and conversation surface. Source, the GitHub release, GitHub Pages, Sites version 19, the installed signed universal app, and the permanent tunnel are live. The Browser and Computer Use boundaries, native Changes review, approved OpenSSH routes, persistent terminals, environments, skills, analysis, and managed worktrees remain intact. Literal parity is not claimed: CodexFlow intentionally does not spawn hidden model agents or automate ChatGPT, and a Windows-native management client remains unbuilt. npm publication and live ChatGPT scheduled/two-chat acceptance remain account-authenticated checks.
...
- The 0.42.0 route-progress regression proves independent web tasks, owner-only persistence across broker restart, lifecycle preservation, clear behavior, secret refusal, and no prompt/tool-content capture; Computer Use visual QA verified the native Tasks hierarchy, progress bars, status pills, and two-column plan at production window size.
...
Every required ledger row remains verified. The full 0.42.0 release matrix passes, and source, GitHub release, GitHub Pages, Sites version 19, the installed signed universal app, and the permanent tunnel are live. This release adds route-scoped native task progress and parallel web-work supervision without creating a second chat, model backend, hidden agent queue, or ChatGPT automation path. npm publication and the account-authenticated ChatGPT acceptance passes remain external checks; the remaining literal-platform gap is a Windows-native management and approval client, while direct model-agent spawning remains intentionally outside the web-first product boundary.
