    ...
    var body: some Scene {
        ...
        .commands {
            CommandMenu("CodexFlow") {
                ...
                Button("Connection") { model.section = .connection }.keyboardShortcut("8", modifiers: .command)
                Button("Policy") { model.section = .policy }.keyboardShortcut("9", modifiers: .command)
                Divider()
